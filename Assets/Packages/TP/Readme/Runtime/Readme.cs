	...
	public class Readme : MonoBehaviour
	{
		...
		public string HtmlText
		{
			get
			{
				...
				//Object
				ReadmeManager.ObjectIdPairs.ForEach(
													pair =>
													{
		...
		public string PreviousRichText { get { return previousRichText; } }

		public TextAreaObjectField[] TextAreaObjectFields { get { return readmeData.textAreaObjectFields; } set { readmeData.textAreaObjectFields = value; } }
		...
		public Dictionary<string, List<bool>> StyleMaps { get { return styleMaps; } }

		public static List<string> SupportedTags { get { return supportedTags; } }

		public ReadmeSettings ActiveSettings { get { return activeSettings; } }
		...
		public void UpdateSettings(bool force = false, bool verbose = false)
		{
			if (!settingsLoaded || force)
			{
				...
				if (allSettings.Count > 0)
				{
					...
					if (!readonlyMode &&
						ActiveSettings.redistributable)
					{
						readonlyMode = true;
					}
		...
		{
			string output = input.Replace("<b></b>", "").Replace("<i></i>", "");
		...
		{
			return richText.Replace("<b>", "").Replace("</b>", "").Replace("<i>", "").Replace("</i>", "");
		...
		public void ToggleStyle(string tag, int startIndex, int length)
		{
			...
			try
			{
				...
				styleMaps[tag] = styleMap.Select(
												(currentIsStyle, index) =>
												{
		...
		public void ApplyStyleMap(string tag)
		{
			...
			RichText = RichText.Replace(Tag(tag), "").Replace(EndTag(tag), "");
		...
		{
			if (richIndex > RichText.Length)
			{
				richIndex = RichText.Length;
			}
		...
		public string Tag(string tagName) { return "<" + tagName + ">"; }

		public string EndTag(string tagName) { return "</" + tagName + ">"; }
	}
}
