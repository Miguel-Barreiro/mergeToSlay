...
namespace MergeToStay.Packages.TP.Readme.Runtime.Settings
{
	[Serializable]
	public struct ReadmeSettings
	{
		...
		public string lastPopupDate { get => LastPopupDate.Instance.value; set => LastPopupDate.Instance.value = value; }
		...
		public bool FileExists() { return File.Exists(FilePath); }

		public static bool PersistantFileExists() { return File.Exists(PersistentFilePath); }

		public static void CreateDefaultSettings()
		{
			ReadmeSettings defaultSettings = new ReadmeSettings("Default", false, true, 7);
			if (!defaultSettings.FileExists() ||
				!PersistantFileExists())
			{
		...
		public static List<ReadmeSettings> LoadAllSettings()
		{
			...
			{
				if (fileInfo.Extension == "." + DEFAULT_TYPE &&
					fileInfo.Name.Substring(0, FILE_TAG.Length) == FILE_TAG)
				{
