...
using MergeToStay.Packages.TP.Readme.Runtime.Settings;
...
namespace TP
{
	...
	public class ReadmeEditor : Editor
	{
		...
		private string RichText { get => readme.RichText; set => readme.RichText = value; }
		...
		private TextAreaObjectField[] TextAreaObjectFields { get => readme.TextAreaObjectFields; set => readme.TextAreaObjectFields = value; }
		...
		private void EditorGuiToolbar()
		{
			...
			if (GUILayout.Button(
								new GUIContent("I", "Italic (alt+i)"),
								italicizedButtonStyle,
								GUILayout.Width(smallButtonWidth)))
			{
				ToggleStyle("i");
			}
			...
			if (GUILayout.Button(
								new GUIContent("Obj", "Insert Object Field (alt+o)"),
								objButtonStyle,
								GUILayout.Width(smallButtonWidth)))
			{
			...
			if (sourceOn)
			{
				EditorGUILayout.HelpBox(
										"Source mode enabled! Supported tags:\n" +
										" <b></b>\n" +
										" <i></i>\n" +
										" <color=\"#00ffff\"></color>\n" +
										" <size=\"20\"></size>\n" +
		...
		private void EditorGuiAdvancedDropdown()
		{
			...
			if (showAdvancedOptions)
			{
				...
				if (showObjectIdPairs)
				{
					...
					GUILayout.BeginHorizontal();
					if (GUILayout.Button("Refresh Pairs", GUILayout.Width(buttonWidth)) ||
		...
		private void UpdateTackIcon(Readme readmeTarget)
		{
			...
			{
				if (readme.useTackIcon &&
					!Readme.neverUseTackIcon)
				{
					Texture2D icon =
		...
		private void CheckKeyboardShortCuts()
		{
			if (ReadmeUtil.UnityInFocus)
			{
				...
				if (editing)
				{
					...
					if (currentEvent.type == EventType.KeyDown &&
						currentEvent.alt &&
						currentEvent.keyCode == KeyCode.B)
					{
						ToggleStyle("b");
						Event.current.Use();
					}
		...
		private void FixCopyBuffer()
		{
			...
			{
				if (EditorGUIUtility.systemCopyBuffer != previousCopyBuffer &&
					previousCopyBuffer != null)
				{
					if (TextEditorActive)
					{
						...
						foreach (string tagPattern in tagPatterns)
						{
							...
							if (textStart >= 0 &&
								RichText.Substring(textStart, textLength) == tagPattern)
							{
		...
		public void ToggleReadOnly() { readme.readonlyMode = !readme.readonlyMode; }

		public void ToggleScroll() { readmeTextArea.ScrollEnabled = !readmeTextArea.ScrollEnabled; }

		public void ToggleEdit() { editing = !editing; }
			...
			if (RichText != null)
			{
				...
				foreach (HtmlNode node in htmlDoc.DocumentNode.Descendants())
				{
					if (node.Name != "#text")
					{
						Debug.Log(
								string.Format(
											"<{0}> Outer - line: {1} startline: {2} outerStart: {3} length: {4}",
											node.Name,
											node.Line,
											node.LinePosition,
											node.OuterStartIndex,
											node.OuterHtml.Length));
						Debug.Log(
								string.Format(
											"<{0}> Inner - line: {1} startline: {2} innerStart: {3} length: {4}",
											node.Name,
											node.Line,
											node.LinePosition,
