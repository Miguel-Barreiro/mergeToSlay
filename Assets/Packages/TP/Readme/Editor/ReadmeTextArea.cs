...
namespace TP
{
	public class ReadmeTextArea
	{
		...
		private string GetName() { return GetName(Editing, SourceOn, Text == ""); }
		...
		public GUIStyle GetGuiStyle(string activeName)
		{
			...
			if (activeName == EmptyName)
			{
				style = emptyRichTextStyle;
			} else if (activeName == ReadonlyName)
			{
				style = selectableRichTextStyle;
			} else if (activeName == StyleName)
			{
				style = editableRichTextStyle;
			} else if (activeName == SourceName)
			{
		...
		public Vector2 GetCursorPixelPosition(int cursorIndex, string text = null)
		{
			...
			if (textAreaRect.x < standardPosition.x &&
				textAreaRect.y < standardPosition.y)
			{
		...
		public bool IsOnTag(int index)
		{
			...
			if (readme != null &&
				readme.richTextTagMap != null &&
				readme.richTextTagMap.Count > index)
			{
				try
				{
		...
		public int GetControlId(string controlName) { return !ControlNameToId.TryGetValue(controlName, out Control control) ? -1 : control.ID; }

		public string GetControlName(int controlId) { return !ControlIdToName.TryGetValue(controlId, out Control control) ? controlId.ToString() : control.Name; }

		public bool HasControl(int controlId) { return ControlIdToName.TryGetValue(controlId, out Control control); }

		public bool HasControl(string controlName) { return ControlNameToId.TryGetValue(controlName, out Control control); }
		...
		private void TriggerUpdateForceEditor(int frameDelay = 0) { updateForceTextEditor = frame + frameDelay; }

		private void TriggerUpdateFocus(int frameDelay = 0) { updateFocusFrame = frame + frameDelay; }

		private void TriggerUpdateReturnFocus(int frameDelay = 0) { updateReturnFocusFrame = frame + frameDelay; }
		...
		private void TriggerOnInspectorGUI() { editor.Repaint(); }
		...
		{
			string newText = Text.Remove(changed.IdIndex, changed.ObjectId.ToString().Length).Insert(changed.IdIndex, changed.ObjectId.ToString());
		...
		private void TriggerUpdateObjectFields(int frameDelay = 2) { updateObjectFieldsFrame = frame + frameDelay; }
