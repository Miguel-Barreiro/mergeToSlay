...
namespace TP
{
	public class ReadmeTextEditor
	{
		...
		private ReadmeTextEditor() { textEditor = TextEditor; }
		...
		private TextEditor GetPrivateTextEditor =>
			typeof(EditorGUI).GetField("activeEditor", BindingFlags.Static | BindingFlags.NonPublic)?.GetValue(null) as TextEditor;
		...
		public void RegisterTextArea(ReadmeTextArea readmeTextArea) { this.readmeTextArea = readmeTextArea; }

		public void Update() { FixCursorBug(); }
		...
		public (int, int) GetCursors() { return textEditor == null ? (-1, -1) : (textEditor.cursorIndex, textEditor.selectIndex); }

		public void BeforeTextAreaChange(ReadmeTextArea readmeTextArea)
		{
			if (!readmeTextArea.TagsError() &&
				!readmeTextArea.SourceOn)
			{
					...
					CursorIndex == SelectIndex)
				{
					...
					{
						if (charIndex < 0 ||
							CursorIndex > readmeTextArea.Text.Length)
						{
		...
		{
			if (ApplyCursorBugFix &&
				TextEditorActive &&
				readmeTextArea.RichTextDisplayed)
			{
		...
		{
			bool mouseEvent =
				new EventType[] {EventType.MouseDown, EventType.MouseDrag, EventType.MouseUp}.Contains(currentEvent.type);

			if (currentEvent.type == EventType.MouseDown &&
				readmeTextArea.Contains(currentEvent.mousePosition))
			{
				mouseCaptured = true;
			}
		...
		public int PositionToIndex(Vector2 position)
		{
			...
			for (int currentIndex = CursorIndex; index == -1; currentIndex = CursorIndex)
			{
				...
				} else if (currentGraphicalPosition.x < goalPosition.x &&
							!isEndOfLine)
				{
					textEditor.MoveRight();
					textEditor.cursorIndex = readmeTextArea.GetNearestPoorTextIndex(CursorIndex);
		...
		{
			get => TextEditor?.controlID ?? -1;
			set
			{
				if (HasTextEditor) TextEditor.controlID = value;
			}
		}
		...
		public string text
		{
			...
			set
			{
				if (HasTextEditor) TextEditor.text = value;
			}
		}

		public int cursorIndex
		{
			get => TextEditor.cursorIndex;
			set
			{
				if (HasTextEditor) TextEditor.cursorIndex = value;
			}
		}

		public int selectIndex
		{
			get => TextEditor.selectIndex;
			set
			{
				if (HasTextEditor) TextEditor.selectIndex = value;
			}
		}

		public Vector2 scrollOffset
		{
			get => TextEditor.scrollOffset;
			set
			{
				if (HasTextEditor) TextEditor.scrollOffset = value;
			}
		}

		public bool multiline
		{
			get => TextEditor.multiline;
			set
			{
				if (HasTextEditor) TextEditor.multiline = value;
			}
		}

		public Rect position
		{
			get => TextEditor.position;
			set
			{
				if (HasTextEditor) TextEditor.position = value;
			}
		}

		public GUIStyle style
		{
			get => TextEditor.style;
			set
			{
				if (HasTextEditor) TextEditor.style = value;
			}
		}

		public int altCursorPosition
		{
			get => TextEditor.altCursorPosition;
			set
			{
				if (HasTextEditor) TextEditor.altCursorPosition = value;
			}
		}

		public Vector2 graphicalSelectCursorPos
		{
			get => TextEditor.graphicalSelectCursorPos;
			set
			{
				if (HasTextEditor) TextEditor.graphicalSelectCursorPos = value;
			}
		}

		public Vector2 graphicalCursorPos
		{
			get => TextEditor.graphicalCursorPos;
			set
			{
				if (HasTextEditor) TextEditor.graphicalCursorPos = value;
			}
		}

		public TextEditor.DblClickSnapping doubleClickSnapping
		{
			get => TextEditor.doubleClickSnapping;
			set
			{
				if (HasTextEditor) TextEditor.doubleClickSnapping = value;
			}
		}

		public bool isPasswordField
		{
			get => TextEditor.isPasswordField;
			set
			{
				if (HasTextEditor) TextEditor.isPasswordField = value;
			}
		}

		public TouchScreenKeyboard keyboardOnScreen
		{
			get => TextEditor.keyboardOnScreen;
			set
			{
				if (HasTextEditor) TextEditor.keyboardOnScreen = value;
			}
		}

		public bool hasHorizontalCursorPos
		{
			get => TextEditor.hasHorizontalCursorPos;
			set
			{
				if (HasTextEditor) TextEditor.hasHorizontalCursorPos = value;
			}
		}
